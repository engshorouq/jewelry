...
if (!empty($_POST['old_password'])) {
    if ($_POST['old'] != $_POST['old_password']) {
        ...
        mysqli_query($conn,"update admin set password='".$_POST['new']."' where id=".$_POST['id']."");
    ...
    if (isset($_FILES["file"]["type"]) and $_POST['flag'] == '2') {
        ...
        if ((($_FILES["file"]["type"] == "image/png") || ($_FILES["file"]["type"] == "image/jpg") || ($_FILES["file"]["type"] == "image/jpeg")) && in_array($ext, $valid_extensions)) {
            ...
            move_uploaded_file($_FILES['file']['tmp_name'], "../assets/admin_img/" . $new_name_image);
        }
        mysqli_query($conn, "update admin set first_name='" . $_POST['first_name'] . "',last_name='" . $_POST['last_name'] . "',photo='" . $new_name_image . "',email='" . $_POST['email'] . "' where id=" . $_POST['id']);
    } else {
        mysqli_query($conn, "update admin set first_name='" . $_POST['first_name'] . "',last_name='" . $_POST['last_name'] . "',email='" . $_POST['email'] . "' where id=" . $_POST['id']);
    }
